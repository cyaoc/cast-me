...
Identity Review first decides completion, then recovery scope. A review passes only when no source-supported Protected Identity Cue shows material, unaccepted drift after accounting for the locked face angle, projection, expression, lighting, styling, and accepted risk. Recognizability alone is insufficient. A clear face, natural pose, clean background, missing watermark, or absent livestream UI proves only that the image is inspectable. Compare the result with the Primary Identity Anchor first, and only after identity passes review hands, text, UI, wardrobe, background, and other module-owned details.
...
If Identity Review fails, decide whether the result remains reliable enough to serve as the visual baseline for one coherent local correction. A defect is local when its correction can be expressed as one coherent affected unit and the surrounding identity relationships remain valid; several symptoms from one face-plane, gaze, mouth, hair, or local styling cause may still be one local defect. Identity-wide drift exists when restoring the person requires rebuilding several underlying facial relationships and the failed result can no longer serve as reliable identity evidence, even if it remains recognizable. Judge structure and repair scope rather than counting symptoms, and do not treat angle-correct asymmetry or an explicitly accepted scoped change as failure.
...
| --- | --- | --- |
| Failed review; reliable identity evidence; one coherent local correction | Surgical edit | Result being edited plus Primary Identity Anchor |
| Failed review; likeness repair plus pose, action, camera, crop, or other whole-image geometry change | New generation from the locked brief | Primary Identity Anchor plus only required reliable supporting person references; exclude the result |
| Failed review; identity-wide drift | New generation from the locked brief | Primary Identity Anchor plus only required reliable supporting person references; exclude the failed result |
