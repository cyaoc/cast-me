...
If the generated output is visible, first review First Read, hierarchy, composition, and intended-use readability at the target display size. Then inspect only relevant high-risk regions at the original available resolution: face identity and geometry; visible hands and prop interactions; and required exact text character by character. Skip absent regions. Preserve angle-correct and natural facial asymmetry rather than treating symmetry as a quality goal. If the output or the resolution needed for a check is unavailable, mark that review unavailable or unverified instead of inferring it from the prompt.
